// 一点万象自动签到（修改这里，这里填是什么签到）
// 20240607 （修改这里）

let sheetNameSubConfig = "ydwx"; // 分配置表名称（修改这里，这里填表的名称，需要和UPDATE文件中的一致，自定义的）
let pushHeader = "【一点万象】";    //（修改这里，这里给自己看的，随便填）
...
// cookie字符串转json格式，格式2，以传入以&连接的字符串
function cookie_to_json2(cookies) {
  var cookie_text = cookies;
  var arr = [];
  var text_to_split = cookie_text.split("&");
  for (var i in text_to_split) {
...
// 一点万象专用sign
function YDWX_SIGN(deviceParams, token, timestamp){
  raw_sign = "action=mixc.app.memberSign.sign&apiVersion=1.0&appId=68a91a5bac6a4f3e91bf4b42856785c6&appVersion=3.58.0&deviceParams=" + deviceParams + "&imei=123456&mallNo=20000&osVersion=15.6&params=eyJtYWxsTm8iOiIyMDAxNCJ9&platform=h5&timestamp=" + timestamp + "&token=" + token + "&P@Gkbu0shTNHjhM!7F"
  // console.log(raw_sign)
  sign = getsign(raw_sign)
  // console.log(sign)
  return sign
}
...
function execHandle(cookie, pos) {
  ...
  url1 = "https://app.mixcapp.com/mixc/gateway"; // 签到url（修改这里，这里填抓包获取到的地址）

  cookieJson = cookie_to_json2(cookie)
  // console.log(cookieJson)
  deviceParams = cookieJson["deviceParams"]
  ...
  data = {
    "mallNo":"20000",
    "appId":"68a91a5bac6a4f3e91bf4b42856785c6", // 固定的
    "platform":"h5",
    "imei":"123456",
    "appVersion":"3.58.0",
    "osVersion":"15.6",
    ...
    "token": token,
    "params":"eyJtYWxsTm8iOiIyMDAxNCJ9",  // 固定的
    "sign": YDWX_SIGN(deviceParams, token, timestamp),
  }
  ...
  if (resp.status == 200) {
    ...
    // {"code":0,"message":"成功","data":{"userPoints":5,"point":5},"timestamp":}
    // {"code":30012,"message":"您的账户异常，请联系商场服务台，或拨打客服热线。（客服时间：工作日9:00-18:00）","data":{},"timestamp":}
    ...
    { 
      // 这里是签到成功
      respdata = resp["data"]
      userPoints = respdata["userPoints"]
      point = respdata["point"]
      content = "签到成功,获得积分:" + point +",当前积分:" + userPoints + " "  // // 给自己看的，双引号内可以随便写
