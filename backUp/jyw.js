// 菁优网自动签到（修改这里，这里填是什么签到）
// 20240608 （修改这里）

let sheetNameSubConfig = "jyw"; // 分配置表名称（修改这里，这里填表的名称，需要和UPDATE文件中的一致，自定义的）
let pushHeader = "【菁优网】";    //（修改这里，这里给自己看的，随便填）
...
function execHandle(cookie, pos) {
  ...
  if (resp.status == 200) {
    ...
    respcode = resp["S"]           // 通过resp["键名"]的方式获取值.假设响应数据是情况1，则读取到数字“0”
    // respmsg = resp["message"]  // 通过resp["键名"]的方式获取值，假设响应数据是情况1，这里取到的值就是“签到成功”
    if(respcode == 0 || respcode == 1)       // 通过code值来判断是不是签到成功，由抓包的情况1知道，0代表签到成功了,所以让code与0比较
    { 
      // 这里是签到成功
      respmsg = resp["M"] 
      content = respmsg + " "//"签到成功 "  // // 给自己看的，双引号内可以随便写
