...
var sheetNameSubConfig = "chouq"; // 分配置表名称， （修改这里）
var pushHeader = "【随机抽签】";    // （修改这里）
...
function resultHandle(resp, pos){
    ...
    const formatDivination = (res) => {
    const { draw, annotate, explain } = res.data;
    const [signText, signLevel] = draw.split(/[()]/).filter(Boolean);
    ...
    return [
      `${signEmoji[signLevel] || '⚪'} ${signText} (${signLevel})`,
      `📜 卦象：${annotate}`,
      `📖 解签：${explain.replace(/　/g, '\n          ')}`, 
      `🌄 典故：${res.data.source.split('。')[0]}...`
    ].join('\n');
  };

    // area = Range("D" + pos).Value2
    let url = "http://shanhe.kim/api/za/chouq.php"
    resp = HTTP.fetch(url, {
        method: "get",
        headers: {},
        // data: data
