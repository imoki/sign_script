...
var sheetNameSubConfig = "oilprice"; // 分配置表名称， （修改这里）
var pushHeader = "【今日油价】";    // （修改这里）
...
function resultHandle(resp, pos){
    ...
    let url = "https://api.03c3.cn/api/oilPrice?area=" + area
    resp = HTTP.fetch(url, {
        method: "get",
        headers: {},
        // data: data
    ...
    const getOilPriceString = (data) => {
      const { area, date, ...prices } = data.data;
      ...
      const priceLines = Object.entries(prices)
        .map(([type, price]) => {
          const label = emojiMap[type] || `⛽${type}号油`;
          return `${label}：${price}元/L`;
        });
    ...
    content = getOilPriceString(oilData)
