...
// 霸王茶姬signature
function sgin(timestamp, userId) {
  let activityId = "947079313798000641";
  let sellerId = 49006
  let params = {
  ...
  let keyValue = Object.keys(params).sort();
  let kv = keyValue.reduce(function (result, item) {
    result[item] = params[item];
    return result;
  }, {});
  var signature = Object.entries(kv).map(function (elem) {
    var [key, value] = elem;
    return key + "=" + value;
  }).join("&") + "&key=" + activityId.split("").reverse().join("");

  // console.log(signature)
  md5Value = getsignUpperCase(signature)
  return md5Value;
}
