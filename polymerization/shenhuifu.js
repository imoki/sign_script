...
var sheetNameSubConfig = "shenhuifu"; // 分配置表名称， （修改这里）
var pushHeader = "【神回复】";    // （修改这里）
...
function resultHandle(resp, pos){
    ...
    const formatGodReply = (data) => {
        const cleanQuestion = data.question.replace(/^问：/, '❓ ');
        const cleanAnswer = data.answer.replace(/^神回复：/, '💡 ');
        
        return `${cleanQuestion}\n${'✨'.repeat(8)}\n${cleanAnswer} 🎇`;
    };

    // area = Range("D" + pos).Value2
    let url = "http://shanhe.kim/api/za/shenhuifu.php"
    resp = HTTP.fetch(url, {
        method: "get",
        headers: {},
        // data: data
