...
let sheetNameSubConfig = "klingai"; // 分配置表名称， （修改这里）
let pushHeader = "【可灵ai】";    // （修改这里）
...
function resultHandle(resp, pos){
    ...
    if(posHttp == 1 || qlSwitch != 1){  // 只在第一次用, 或者执行金山文档
          ...
          // {"status":200,"message":"成功","data":{"order":null,"status":"GRANTED"},"timestamp":[2024,7,24,16,56,1,822322000]}
          ...
          respstatus = resp["status"] // 通过resp["键名"]的方式获取值.假设响应数据是情况1，则读取到数字“0”

          if(respstatus == 200 )   // 通过code值来判断是不是签到成功，由抓包的情况1知道，0代表签到成功了,所以让code与0比较
          {
              content = "🎉 签到成功" + ""
    ...
    if(posHttp == 2 || qlSwitch != 1){  // 只在第2次用, 或者执行金山文档
      // {"status":200,"message":"成功","data":{"points":[{"orderId":"","type":"reward","amount":6600,"balance":6600,"startTime":,"endTime":}],"total":6600},"timestamp":}
      resp = resp.json(); // 返回json格式则resp.json()。否则为resp.text()，此时就要用正则处理响应
      console.log(resp)
        ...
        if(respstatus == 200 )   // 通过code值来判断是不是签到成功，由抓包的情况1知道，0代表签到成功了,所以让code与0比较
        {
            points = resp["data"]["total"]
            content = "🎉 当前灵感值：" + points +  ""
            messageSuccess += content;
            // console.log(content)
...
function execHandle(cookie, pos) {
    ...
    let url = "https://klingai.kuaishou.com/api/pay/reward?activity=login_bonus_daily"; // 签到url（修改这里，这里填抓包获取到的地址）
