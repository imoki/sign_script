...
var sheetNameSubConfig = "rsdjs"; // 分配置表名称， （修改这里）
var pushHeader = "【人生倒计时】";    // （修改这里）
...
function resultHandle(resp, pos){
    ...
    const formatLifeTime = (data) => {
        const progressBar = (current, total, width=12) => {
            const filled = '█'.repeat(Math.round(current/total*width));
            return `${filled.padEnd(width, '░')} ${(current/total*100).toFixed(1)}%`;
        };
        
        return [
            `📅 ${data.month.replace(/\D+/g,'')}/${12}个月 ${progressBar(4, 12)}`,
            `🔄 ${data.week.replace(/\D+/g,'')}/7周 ${'⏳'.repeat(7)}`,
            `🌞 ${data.day.replace(/\D+/g,'')}/30天 ${progressBar(6, 30, 8)}`,
            `⏰ ${data.hour.match(/\d+/g).slice(0,3).join(':')} ${'🕒'.repeat(3)}`,
            `📌 最后更新：${data.time.split(' ')[1].slice(0,5)}`
        ].join('\n');
    };

    // area = Range("D" + pos).Value2
    let url = "http://shanhe.kim/api/za/rsdjs.php"
    resp = HTTP.fetch(url, {
        method: "get",
        headers: {},
        // data: data
