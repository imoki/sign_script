...
var sheetNameSubConfig = "lnglat"; // 分配置表名称， （修改这里）
var pushHeader = "【经纬度解析】";    // （修改这里）
...
function resultHandle(resp, pos){
    ...
    lng = Range('D'+pos).Value2   // 经度 longitude
    lat = Range('E'+pos).Value2   // 纬度 latitude
    let url = "https://api.suyanw.cn/api/location_geocoder_address.php?lng=" + lng + "&lat=" + lat
    resp = HTTP.fetch(url, {
        method: "get",
        headers: {},
        // data: data
    ...
    output = resp
    console.log(output)
    Application.Range('F'+pos).Value2  = output
