...
//     </div>
//     <div class="fdTime">
//         Last change:<span style='font-weight:bold;color:#FF0000;'> Saturday <span dir='ltr'>7</span> September 2024 11:36:00 (UTC)</span>
//     </div>
// </div>
...
// 正则匹配
        Reg = [
        // /;">(.+?)</i,
        /<span style="color: #00b050;">(.+?)<\/span>/i,
        ]
        ...
        // 去掉回车换行
        multiLineArray = html.split('\r\n');
        html = multiLineArray.join('');
        // console.log(html)
        ...
        for(i=0; i< Reg.length; i++)
        {
            ...
            if (flagTrue == true) {
                ...
                if(qlSwitch == 0){
                  // 金山文档
                  newCode = result
                  oldCode = Application.Range("D" + pos).Text // 旧
                  if(newCode == oldCode){
                    content = "📢 优惠码没变化 "  + valueName[i] + ":" + result + valueEnd[i] + " "
                    messageSuccess += content;
                    // console.log(content)
                    console.log("📢 不推送")
                  }else{
                    // console.log("写入最新的")
                    Application.Range("D" + pos).Value = newCode // 将旧的覆盖
                    content = "🎉 " + valueName[i] + ":" + result + valueEnd[i] + " "
                    messageSuccess += content;
                    push(messageSuccess); // 推送消息
                  }
                }
