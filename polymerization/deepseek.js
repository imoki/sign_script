...
var sheetNameSubConfig = "deepseek"; // 分配置表名称， （修改这里）
var pushHeader = "【deepseek分析】";    // （修改这里）
...
function getlog() {
  ...
  for (let i = 2; i <= 100; i++) {
      ...
      msgDict.live = Application.Range("E" + i).Text;    // 是否存货
    ...
    // 将 msgArray 转换为纯文本日志
    log = convertMsgArrayToLog(msgArray);
...
// 将 msgArray 转换为纯文本日志
function convertMsgArrayToLog(msgArray) {
    let log = "";
    const MAX_MSG_LENGTH = 100; // 定义消息的最大长度
    // 获取当前日期
    const today = new Date();
    const todayFormatted = formatDate(today); // 格式化为 YYYY/MM/DD
    ...
    msgArray.forEach(msg => {
        ...
        // 检查 msg.flagPush 是否为 否
        if (msg.flagPush == "否" || msg.live == "否") {
            return; // 跳过该条消息
        }

        // 检查运行时间是否为当天
        const runDate = formatDate(new Date(msg.update)); // 格式化运行时间为 YYYY/MM/DD
        if (runDate !== todayFormatted) {
            return; // 跳过非当天的消息
        }

        // 排除部分
        if(msg.name == "deepseek") {
          return;
        }
        ...
        log += `待推送消息: ${msg.msg}\n`;
        log += `推送时间: ${msg.date}\n`;
        // log += `推送方式: ${msg.methodPush}\n`;
        // log += `是否通知: ${msg.flagPush}\n`;
        // log += `是否加入消息池: ${msg.pool}\n`;
        // log += `优先级: ${msg.priority}\n`;
        // log += `当日可推送次数: ${msg.dailyPushLimit}\n`;
        // log += `当日剩余推送次数: ${msg.remainingDailyPushes}\n`;
        log += "\n"; // 每条消息之间空一行
    });
