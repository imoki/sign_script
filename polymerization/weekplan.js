...
var sheetNameSubConfig = "weekplan"; // 分配置表名称， （修改这里）
var pushHeader = "【周安排】";    // （修改这里）
...
function resultHandle(resp, pos){
    ...
    weekPlanContent = []
    for (let i = 0; i < 7; i++) {
      let currentPos = colNum[i + 3] + "" + pos
      // console.log(currentPos)
      weekPlanContent[i] = Range(currentPos).Value2
    }
    
    // 获得今天星期几, 返回值范围 0（周日）-6（周六）
    // 0: 周日 | 1: 周一 | 2: 周二 | ... | 6: 周六
    let today = new Date().getDay();
    // 或转换为国际标准（周一=0）
    const mondayFirst = (today + 6) % 7;
    // console.log(mondayFirst); // 周一=0, ..., 周日=6
    message =  weekPlanContent[mondayFirst]
    // console.log(message)
    if (message != "") {
      pushDirect(message)
    }
