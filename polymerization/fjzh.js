...
var sheetNameSubConfig = "fjzh"; // 分配置表名称， （修改这里）
var pushHeader = "【简繁体转换】";    // （修改这里）
...
function formatPhoneInfo(response) {
    ...
    const { phone, province, city, operator } = data || {};
    
    // 错误处理：基础校验
    if (!data || province === null || operator === null) {
        return `❌ 无效手机号：${phone}`;
    }
    ...
    const location = [
        province,
        city && city !== province ? city : null, // 避免重复显示相同城市
        operator
    ].filter(Boolean).join('·');

    return `✅ 归属地：${location} 📱${phone}`;
}
...
function resultHandle(resp, pos){
    ...
    let url = "https://api.suyanw.cn/api/jfzh.php?text=" + input
    resp = HTTP.fetch(url, {
        method: "get",
        headers: {},
        // data: data
