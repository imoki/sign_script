...
let sheetNameSubConfig = "xpnc"; // 分配置表名称， （修改这里）
let pushHeader = "【兴攀农场】";    // （修改这里）
...
function resultHandle(resp, pos){
    ...
    if (resp.code == "1000") {
      rewardintegral = resp["data"]["reward"]["integral"]
      content = "🎉 签到成功,获得" + rewardintegral +  "积分" + "\n"
      messageSuccess += content;
      flagSign = 1

    }else if(resp.code == "10200"){
      // {"code":10200,"message":"已经签过到了，明天再来","request_url":"/treemp/user.PersonalCenter/addSignIn","success":false,"data":[]}
    ...
    if(flagSign == 1){
      ...
      for (let id = 1; id <= 13; id++) {
        ...
        if (resp.code == "1000" || resp.code == "10200") {
          ...
          if (resp.code == "1000") {
            ...
            respmessage = resp["message"]
            reward_type_name = resp["data"]["reward"][0]["reward_type_name"]
            reward = resp["data"]["reward"][0]["reward"]
            content = "🎉 任务" + id +  "获得" + reward_type_name + reward + "\n"
            messageSuccess += content;

          }else if(resp.code == "10200"){
            // {"code":10200,"message":"奖励任务已完成","request_url":"/treemp/tree.Tasks/completeTask","success":false,"data":[]}
            respmessage = resp["message"]
            content = "📢 任务" + id + respmessage + "\n"
            messageSuccess += content;
          }else{
            respmsg = resp["message"]
            content = "❌ 任务" + id + "领取失败:" + respmsg  + "\n"
            messageFail += content;
          }
      ...
      // 最多浇水20次
      countMax = 20  // 最大浇水次数
      countDynamic = 20 // 根据响应动态变化
      countReal = 0 // 记录实际浇水次数
      for (let i = 0; i < countMax && i < countDynamic; i++) {
