...
let sheetNameSubConfig = "rainyun"; // 分配置表名称， （修改这里）
let pushHeader = "【雨云】";    // （修改这里）
...
function resultHandle(resp, pos){
    ...
    if (resp.status == 200) {
      ...
      for(i=0; i<setcookie.length; i++){
        cookie += setcookie[i]
        cookieJson = cookie_to_json(setcookie[i]) 
        // token = cookieJson["X-CSRF-Token"]
        // if(token != undefined){
        //   console.log("已捕捉到X-CSRF-Token")
        //   // break
        // }
        jsonObj = cookieJson
        for(key in jsonObj){ 
          // console.log(key)
          headers[key] = jsonObj[key]
        } 
      }
      ...
      // {"code":10012,"message":"CSRFToken 验证失败"}
      // {"code":30002,"message":"需要登录"}
      // {"code":30011,"message":"未达到完成这个任务的条件，继续加油哦！"}
      // {"code":200,"data":"ok"}
      respcode = resp["code"]           // 通过resp["键名"]的方式获取值.假设响应数据是情况1，则读取到数字“0”
      respmsg = resp["message"]  // 通过resp["键名"]的方式获取值，假设响应数据是情况1，这里取到的值就是“签到成功”
      if(respcode == 200)       // 通过code值来判断是不是签到成功，由抓包的情况1知道，0代表签到成功了,所以让code与0比较
      { 
        // 这里是签到成功
      ...
      {
        // 这里是签到失败
        // respmsg = "签到失败 "   // 给自己看的，可以随便写，如 msg = "失败啦！ " 。
...
function execHandle(cookie, pos) {
    ...
    // // 登录，获取X-CSRF-Token
    url = "https://api.v2.rainyun.com/user/login"; // 签到url（修改这里，这里填抓包获取到的地址）
