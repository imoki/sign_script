/*
    ...
    更新时间：20240406
    环境变量名：todayhistory（修改这里）
    环境变量值：无（修改这里）
    备注：历史上的今天
*/

const logo = "艾默库 : https://github.com/imoki/sign_script"    // 仓库地址
var sheetNameSubConfig = "todayhistory"; // 分配置表名称， （修改这里）
var pushHeader = "【历史上的今天】";    // （修改这里）
...
function resultHandle(resp, pos){
    ...
    let url = "https://api.03c3.cn/api/history"
    resp = HTTP.fetch(url, {
        method: "get",
        headers: {},
        // data: data
    });
    historyData = resp.json()

    // console.log(historyData)
    
    const getEventEmoji = (desc) => {
      const emojiMap = {
        逝世: '⚪', 牺牲: '⚪', 病逝: '⚪',
        诞生: '👶', 出生: '👶', 成立: '🏛️',
        战争: '⚔️', 宣战: '⚔️', 入侵: '⚔️',
      ...
      const matched = Object.entries(emojiMap)
        .find(([key]) => desc.includes(key));
  ...
  const formatHistory = (data) => 
    data.map(event => {
      const emoji = getEventEmoji(event.description);
      return `${emoji} ${event.year} - ${event.description}`;
    }).join('\n');

  content = formatHistory(historyData.data)
