...
var sheetNameSubConfig = "steamrank"; // 分配置表名称， （修改这里）
var pushHeader = "【steam游戏在线人数】";    // （修改这里）
...
function resultHandle(resp, pos){
    ...
    const formatSteamStats = (data) => {
    const stats = [
      `🎮 ${data.name_cn}`,
      `👥 当前玩家：${Number(data.current).toLocaleString()} (排名#${data.current_rank})`,
      `🚀 历史峰值：${Number(data.peak).toLocaleString()} (${new Date(data.peak_t*1000).getFullYear()}年记录)`,
      `📈 24小时峰值：${Number(data.day_peak).toLocaleString()} (排名#${data.day_peak_rank})`,
      `🕒 最后更新：${new Date(data.update_time).toLocaleTimeString('zh-CN')}`
    ];
    
    // 添加特别成就标识
    if(data.peak_rank === "1") stats.push(`🏆 历史峰值全站第1名!`);
    if(data.day_peak_rank <= 3) stats.push(`🔥 今日热度TOP${data.day_peak_rank}`);
    ...
    let url = "https://jkapi.com/api/steam_rank?appid=" + input
    resp = HTTP.fetch(url, {
        method: "get",
        headers: {},
        // data: data
