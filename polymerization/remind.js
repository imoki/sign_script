...
var sheetNameSubConfig = "remind"; // 分配置表名称， （修改这里）
var pushHeader = "【日期提醒】";    // （修改这里）
...
function resultHandle(resp, pos){
    ...
    let month = now.getMonth() + 1; // 将月份转换为实际的月份（1 - 12）
    let day = now.getDate();
    let riqi = `${year}年${month}月${day}日`

    let today = Range('D'+(i+1)).Value2
    let jinnian = formatDate(today).substring(0, 4)
    let type = Range('F'+(i+1)).Value2
    let 月 = Range('G'+(i+1)).Value2
    let 日 = Range('H'+(i+1)).Value2
    let title = Range('E'+(i+1)).Value2

    if(year != jinnian){
        if(type=='阳历'){
        targetRange = Range("D"+(i+1))
        targetRange.Value2 =(`${year}年${月}月${日}日`)
        }

        if(type=='农历'){
        let resp = HTTP.get(`https://www.iamwawa.cn/nongli/api/?type=lunar&year=${year}&month=${月}&day=${日}`, {
        headers: { "User-Agent": "iamwawa-open-api" }
        });
        const data = resp.json()
        targetRange = Range("D"+(i+1))
        targetRange.Value2 = data.data.solar
        }

    }

    if(type=='每月'){
      let tixinri = Range('H'+(i+1)).Value2
      today = `${year}年${month}月${tixinri}日`
      console.log(today)
    }else{
      today = Range('D'+(i+1)).Value2
      today =formatDate(today)
    }

    // 判断是否推送
    if(today!=riqi){
      // HTTP.get(bark+'/WPS日期提醒/'+title+'?icon='+icon, {
      //     headers: { "Content-Type": "application/x-www-form-urlencoded" },
      // });
      message = title 
      pushDirect(message)
...
function formatDate(num) {
    // 以1900年1月1日为基准，因为Excel等软件中日期计数以此为起点
    let baseDate = new Date(1900, 0, 1);
    // 根据传入的数值计算出对应的日期
    let targetDate = new Date(baseDate.getTime() + (num - 1) * 24 * 60 * 60 * 1000);
    let year1 = targetDate.getFullYear();
    let month1 = targetDate.getMonth() + 1;
    let day1 = targetDate.getDate();
