/*
    ...
    更新时间：20240923
    备注：需要cookie。
          cookie填写夸克网盘网页版中获取的cookie。F12 -> "NetWork"(中文名叫"网络") -> 按一下Ctrl+R -> list -> Cookie
*/

const logo = "艾默库 : https://github.com/imoki/sign_script"    // 仓库地址
var sheetNameSubConfig = "quarksave"; // 分配置表名称， （修改这里）
var pushHeader = "【夸克订阅更新自动转存】";    // （修改这里）
...
// ======================夸克处理相关开始======================
function quark(cookie){
  let msgmain = []
  let result = ""

  console.log("🧩 开始更新订阅");
  let shareUpdateList = getShareUpdateList(cookie);

  for (let i = 0; i < shareUpdateList.length; i++) {
    let elem = shareUpdateList[i];
    console.log(`📑 订阅：${elem.title}`);
    if (elem.save_as_status == 0) {
      let dirdata = getSharepageDir(cookie, {target_fid: elem.first_fid,pwd_id: elem.pwd_id,});
      sharepageSave(cookie, { pwd_id: elem.pwd_id, stoken: elem.stoken, to_pdir_fid: dirdata.pdir_fid,});
      result += `🎉 订阅：${elem.title} 更新完成\n`
      
    } else {
      result += `🎉 订阅：${elem.title} 无需更新\n`
      // console.log(result)
    }
  }
  // console.log(result)
...
// 获取更新列表
function getShareUpdateList(cookie) {
  params = "?pr=ucpro&fr=pc&uc_param_str="
  url = "https://drive-pc.quark.cn/1/clouddrive/share/update_list" + params
  ...
  respdata = resp["data"]["list"]
  // console.log(respdata)
  return respdata
}

// 获取分享页目录
function getSharepageDir(cookie, data) {
  params = "?pr=ucpro&fr=pc&aver=1&scene=&uc_param_str=&target_fid=" + data.target_fid + "&pwd_id=" + data.pwd_id
  url = "https://drive-pc.quark.cn/1/clouddrive/share/sharepage/dir" + params
  // console.log(url)
  headers = {
    "content-type": "application/json",
    "cookie": cookie,
...
// 转存文件
function sharepageSave(cookie, data) {
  params = "?entry=update_share&pr=ucpro&fr=pc&uc_param_str="
  url = "https://drive-pc.quark.cn/1/clouddrive/share/sharepage/save" + params
