/*
    ...
    更新时间：20240815
    环境变量名：oray
    环境变量值：填写“向日葵”app抓包得到的cookie
*/

const logo = "艾默库 : https://github.com/imoki/sign_script"    // 仓库地址
let sheetNameSubConfig = "oray"; // 分配置表名称， （修改这里）
let pushHeader = "【贝锐阳光小店】";    // （修改这里）
...
function resultHandle(resp, pos){
    ...
    if(posHttp == 1 || qlSwitch != 1){  // 第1次进来时用
      if (resp.status == 200) {
        ...
        dialogtitle = arguments["dialogtitle"]  // 签到成功
        dialogdesc = arguments["dialogdesc"]  // 获得20阳光

        content = "🎉 "+ dialogtitle + "，" + dialogdesc + ""
        messageSuccess += content;
        // console.log(content)

        // 查积分
        url = "https://store-api.oray.com/point/balance"; // 积分url（修改这里，这里填抓包获取到的地址）
        // headers= {
        //   "Cookie": cookie,
    ...
    if(posHttp == 2 || qlSwitch != 1){  // 第2次进来时用
      // console.log(resp.text())
      if (resp.status == 200) {
...
function execHandle(cookie, pos) {
    ...
    let url = "https://store-api.oray.com/points/sign"; // 签到url（修改这里，这里填抓包获取到的地址）
