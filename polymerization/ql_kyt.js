...
function resultHandle(resp, pos){
    ...
    if (resp.status == 200) {
        ...
        if(respcode == 0 )   // 通过code值来判断是不是成功，由抓包的情况1知道，0代表成功了,所以让code与0比较
        {
            respmsg = resp["msg"]
            content = "🎉 " + respmsg + ""
            ...
            respmsg = resp["msg"]   // 通过resp["键名"]的方式获取值，假设响应数据是情况1，这里取到的值就是“成功”
            content = "📢 " + respmsg + ""
            messageFail += content;
            // if(respmsg == "请先登录"){
            //     content = "❌ " + respmsg + ""
...
function execHandle(cookie, pos) {
    ...
    let url = "https://www.ablesci.com/user/sign"; // url（修改这里，这里填抓包获取到的地址）
